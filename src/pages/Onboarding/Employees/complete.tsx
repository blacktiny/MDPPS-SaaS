...
const EmployeesOnboardingComplete: React.FC<RouteComponentProps> = () => {
  ...
  const isInvited = useMemo(() => {
    if (user) return user.initial_account_type === 'employee' && user.account_type === user.initial_account_type
    return false
  }, [user])

  const companyName = useMemo(() => {
    if (user && user.companies && user.companies.length > 0) return user.companies[0].business_name
    return 'false'
  }, [user])
      ...
      <QuestionsTemplate steps={steps} activeStep={activeStep}>
        <CompleteWrapper>
          <section style={{ maxWidth: boxModel('614px') }}>
            ...
            <Message>
              {isInvited
                ? `Your new account has been successfully created for ${companyName}. Navigate to the dashboard to begin
              accessing your team’s data and utilizing our suite of intelligent apps to assist in your assigned role.`
                : `You’ve successfully created a new account and your authorization request has been submitted to the administrator of company ${companyName}. While you wait, navigate to the dashboard to utilize our suite of intelligent apps.`}
            </Message>
...
export default EmployeesOnboardingComplete
